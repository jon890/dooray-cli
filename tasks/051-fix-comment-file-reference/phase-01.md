...
계획 보완 중 `origin/main`은 README 뱃지만 복원한 `4e2b1b0`까지 진행됐다.
phase 02의 삽입 기준인 `에이전트 없이 직접 쓰기`와 `프로젝트 구조` 사이는 유지된다.
`2a660ec`은 최소 기준선일 뿐 고정 대상이 아니다.
구현 작업트리는 최신 `origin/main`으로 재기반한다.
`README.md`와 공개 스킬 문서의 겹치는 변경도 다시 확인한다.
...
- Dooray가 일반 링크 `/files/<fileId>`를 클릭 가능한 파일로 렌더하지 않는다는 새 실측 근거가 나오면 다음처럼 처리한다.
  - `PHASE_BLOCKED: 일반 링크 렌더 계약 재검토 필요`를 보고한다.
  - 범위를 임의로 넓히지 않는다.
