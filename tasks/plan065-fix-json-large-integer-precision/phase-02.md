...
**이 phase 는 메시지를 보내지 않는다.** 정밀도가 실제로 보존되는지는
phase 01 의 파서 단위 테스트와 아래 항목 6 의 `client.test.ts` 가 판정한다.
그 테스트는 `fetch` 를 흉내 내므로 서버로 나가지 않는다.

실제 발송으로 확인하고 싶으면 사람이 대상을 직접 정해서 실행한다.
이 문서에 대상 자리를 비워 두면 실행하는 쪽이 그 자리를 채우게 되고,
메신저 발송은 되돌리기 어렵다.
