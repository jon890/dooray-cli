...
- `dooray wiki` — 목록·페이지 조회·생성·수정($EDITOR)·이동·트리 조회 (`wiki tree` 레벨별 재귀 drill-down, ADR-034, Issue #101, 이동은 ADR-047). 목록은 `--search` 로 위키 이름을 대소문자 무시 부분 일치로 찾고 project 를 함께 보여준다 (ADR-043, Issue #154)
