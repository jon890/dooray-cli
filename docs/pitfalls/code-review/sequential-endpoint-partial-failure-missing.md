---
id: sequential-endpoint-partial-failure-missing
category: code-review
title: sequential endpoint 호출 — partial-failure stderr 안내와 spinner pair 누락
triggers: [sequential 호출, partial-failure, stopSpinner, re-throw]
tool_catchable: false
source: [CLI23, PR #62]
related: []
---

**증상**: 본문 변경(`updatePost`)과 메타데이터 변경(`setPostParent` / `setPostWorkflow` / `deletePostFile` 호출 후 `updatePost`로 댓글 PUT 합성 등) 을 sequential 로 호출.
atomic 보장 없으므로 첫 호출 성공 후 두 번째 실패 시 부분 상태 발생.
...
phase 본문 작업 항목에 try/catch와 stderr 안내 코드 스니펫 명시.
