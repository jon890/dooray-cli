...
tmp 가 이전 실행에서 남아 있으면 `mode` 가 적용되지 않으니 `chmod(tmp, 0o600)` 을 한 번 더 부르고, 실패는 무시한다(Windows).
`~/.dooray` 디렉터리는 `mkdir(..., { recursive: true, mode: 0o700 })` 으로 만든다. `src/config/store.ts` 의 `writeConfigFile` 이 이 형태다.
