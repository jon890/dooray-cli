...
triggers: [캐시 무효화, clearTags, mutation, TTL, resolver 쓰기 함수, ADR-042]
...
**Good**: 쓰기 호출을 `src/resolvers/` 의 쓰기 함수로 감싸고, 그 함수가 성공 직후 캐시 파일을 지운다.
캐시를 읽는 계열이 무효화도 맡으므로 호출자가 잊을 수 없다.
명령 파일이 `api/client` 를 직접 부르고 따로 캐시를 지우는 형태를 만들지 않는다.
