---
id: stale-interactive-warning-after-trigger-expansion
category: plan
...
**증상**: `nonInteractive` 진입 조건에 새 옵션을 추가 (`|| hasTagChange` 등).
  ...
  새 옵션이 trigger 에 포함됐으므로 else 분기에서는 절대 true 가 안 됨 → **dead code와 메시지가 사실과 반대** (단독 호출이 이번 기능의 핵심인데 "단독 호출 안 됨" 안내 출력 가능성 0이지만 의도 충돌).
