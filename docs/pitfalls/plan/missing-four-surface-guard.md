---
id: missing-four-surface-guard
category: plan
title: 새 불변식 도입 시 4면 가드 누락
triggers: [불변식, cache writer, resolver, formatter, schema]
tool_catchable: false
source: [1-7]
related: []
---

**증상**: 캐시 스키마에 신규 필드를 추가하고 일부 read 경로에만 가드를 넣은 뒤 writer는 누락.
...
2. **Cache writer & reader**: `src/cache/store.ts` 양쪽 모두 신 필드 처리와 atomic write
