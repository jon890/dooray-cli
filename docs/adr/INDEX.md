...
- [ADR-008](008-member-ambiguity-error-candidates.md) — 멤버 모호성: 에러 + 후보 출력
...
- [ADR-020](020-post-input-unification-vitest.md) — post 명령 input 통합 (`--id`/URL/positional) + 첫 테스트 인프라 (vitest)
- [ADR-021](021-member-command-creator-enrich.md) — `member` 명령 + `comment list` Creator 이름 자동 채우기
- [ADR-022](022-feedback-gh-cli.md) — `dooray feedback` 명령 + GitHub 호출은 `gh` CLI 위임
- [ADR-023](023-feedback-last-run-tracking.md) — `dooray feedback --last` last-run 추적 (opt-in + 에러시만 + 최소 세트 + argv 패턴 마스킹)
- [ADR-024](024-comment-file-synthesis.md) — `dooray post comment file *` (post-level files API + 댓글 PUT 합성)
- [ADR-025](025-post-cc-to-member-group.md) — `post edit/create` cc/to 에 member-group 추가 (full payload PUT + `type: "group"`)
...
- [ADR-028](028-member-group-response-shape.md) — member-group 응답 shape — nested array unwrap + id 직접 입력 fallback (Issue #65, #76)
