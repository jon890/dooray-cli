...
`--workflow` 만 create 후 `setPostWorkflow` 후속 호출 — 실패 시 `stderr` warn 후 `exit 0` (post 는 이미 생성됨).
...
세부 시그니처·동작은 `src/commands/post/create.ts` 와 `src/resolvers/{tag,milestone,postRef}.ts` 참조.
캐시 디렉터리는 `data-schema.md`.

**확장 (2026-05-18, Issue #66)**: `post edit` 도 동일 정책 적용 — `--tag` / `--tag-clear` / `--tag-remove` 옵션과 mandatory 검증 동일 호출.
