...
- 하위 페이지 있으면 차단하고 `--recursive` 제공 — 하위가 orphan 없이 재부착되므로 차단은 과보호. 플래그·서버 동작 가정만 늘림.
...
- 입력 해석은 `resolveWikiPageInput` 재사용 (`<project> <page-id>` / `--id`와 `--project` / `--url` / positional URL).
