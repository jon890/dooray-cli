...
| **docs-verifier** | `dooray-cli-docs-verifier` (custom, project-local at `.claude/agents/`) | sonnet | 코드↔docs 정합성 검증 (PASS/UPDATE_NEEDED/VIOLATION). dooray-cli 도메인 지식 (ADR-001~024 / docs 영향 표 / 캐시 규약 / 개인 식별 정보 사전 점검) 자동 적용 — 매번 검사 항목 길게 전달 불요 |
...
7. **`docs/pitfalls/plan/` + `docs/pitfalls/team/` 의 모든 패턴이 사전 해소되었는가?**
...
**사전 해소 점검 (필수)**: code-reviewer 검사 시작 전에 `docs/pitfalls/code-review/` 의 모든 패턴 파일이 코드에 적용됐는지 확인.
적용 안 됐으면 그 자리에서 FIX_NEEDED 회신 (executor 재투입).
`docs/pitfalls/INDEX.md` 가 회피 패턴의 단일 소스 (라우터) — 카테고리 디렉터리 전체를 grep 점검.
...
7. **exitCode 누락**: catch 블록에서 `process.exit(N)` 또는 `throw new DoorayCliError(msg, N)` 누락 시 호출 스크립트가 실패 인지 못함 — grep으로 catch 패턴 검증
...
10. **캐시 atomic write**: `~/.dooray/cache/` 쓰기는 temp 파일 + rename 패턴 (race 방지). `writeFile` 직접 호출 금지
...
7. **planning docs 영향 표 100% 적용 검증** — `.claude/planning-overlay.md` "변경 유형별 docs 영향 표" 의 해당 행 식별 + 표시된 모든 docs 갱신 확인
...
7. **review 회고 (조건부 필수 — 학습 루프)** — PR 생성 직후, 팀 shutdown 직전
   ...
   - **critic** REVISE 지적 중 *반복 가능성* 있는 패턴 → `docs/pitfalls/plan/` 또는 `docs/pitfalls/team/` 해당 카테고리 디렉터리에 새 패턴 파일 생성
   - **code-reviewer** FIX_NEEDED 지적 중 *반복 가능성* 있는 패턴 → `docs/pitfalls/code-review/` 에 새 패턴 파일 생성 (또는 새 카테고리 디렉터리 신설)
