...
자연어 요청을 dooray-cli 커맨드로 옮길 때 쓰는 표다. 필요한 영역의 절만 읽는다.

대상 지정 방법(`--id` / `--url` / positional URL)과 파일 명령의 `--json` 스키마는 [SKILL.md](../SKILL.md) 에 있다.
...
| organization 전체 멤버 검색 | `dooray member search <keyword>` — 옵션은 [common.md](common.md) |
...
| 의도 | 커맨드 |
| --- | --- |
| 댓글 조회 | `dooray post comment list <project> <number>` — 필터는 [comment.md](comment.md) |
...
`--json` 출력 스키마는 [post.md](post.md) 를 읽는다.
